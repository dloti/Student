...
void Equality::UpdateDenotations() {
	std::vector<std::vector<std::pair<int, int> > > lDenot =
			this->left->GetDenotationRoleVec();
	std::vector<std::vector<std::pair<int, int> > > rDenot =
			this->right->GetDenotationRoleVec();
	...
	for (unsigned i = 0; i < lDenot.size(); ++i) {
		...
		std::vector<std::pair<int, int> >::iterator it = rDenot[i].begin(),
				last = rDenot[i].end(), it1 = lDenot[i].begin(), last1 =
						lDenot[i].end();
		...
		for (; it != last; ++it) {
			for (it1 = tmp; it1 != last1; ++it1) {
				if (it->first == it1->first && it->second == it1->second
						&& std::find(tmpInterpretation.begin(),
								tmpInterpretation.end(), it->first)
...
void Equality::UpdateInterpretation() {
	...
	std::vector<std::pair<int, int> >::iterator it =
			...
			it1 = leftInterpretation->begin(), last1 =
	...
	for (; it != last; ++it) {
		for (it1 = tmp; it1 != last1; ++it1) {
			if (it->first == it1->first && it->second == it1->second
					&& std::find(this->interpretation.begin(),
							this->interpretation.end(), it->first)
